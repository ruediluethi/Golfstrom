...
\begin{document}
	...
	\abstract{In diesem Bericht wird der Golfstrom als mathematisches Modell dargestellt. Mit diesem Modell werden Gleichgewichtsuntersuchen und Stabilitätsanalysen durchgeführt. Abschließend untersuchen wir des Verhalten des Golfstroms anhand von realen Werten und analysieren was passiert, wenn es zu einer Klimaerwärmung kommt und die Arktis zu schmelzen beginnt oder in wie fern sich die Jahreszeiten auf den Golfstrom auswirken.}
...
Im Nordmeer fällt das Wasser in eine Tiefe von über 2000 Metern.
	...
	\begin{figure}[!h]
 		...
 		\includegraphics[width=7cm]{../Diagramme/zustandsdiagram.png}
  		\caption{Zustandstrajektorien für das Beispiel i. aus Abbildung \ref{fig:q_von_q_falluntersuchung}. Für den Plot wurden zufällige Startwerte für \(T\) und \(S\) gewählt. Aus der Ableitung \(g'\) wissen wir ob es sich um stabile oder instabile Punkte handelt: \(g'(q_1) \approx -5.4 < 1 \Rightarrow \textrm{stabil}, g'(q_2) \approx 2.8 > 1 \Rightarrow \textrm{instabil}, g'(q_3) \approx 0.2 \Rightarrow < 1 \Rightarrow \textrm{stabil} \). }
